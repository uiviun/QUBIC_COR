...
void make_graph (const char* fn)
{
	...
	{	
		for (j = 0; j < Trows; j++)
		{
			for (i = 0; i < rows; i++)
			{
			cnt = str_intersect_r(arr_c[i], arr_c[T_list[j]]);
			cnt1 = str_intersect_r(arr_c[i], arr_c[i]);
			cnt2 = str_intersect_r(arr_c[T_list[j]], arr_c[T_list[j]]);
			if (po->IS_spearman && cnt>5)
			{
				spearman = get_spearman (arr_c[i],arr_c[T_list[j]],i,T_list[j],cnt);
				fscore = get_f_socre (cnt, cnt1, cnt2);
				cnt3 = cnt;
				cnt = ceil (2*cnt3*spearman*fscore);
				final = 2*cnt3*spearman*fscore;
				printf ("%s\t%s\t%d\t%d\t%d\t%.2f\t%.2f\t%.2f\n",genes_n[i],genes_n[T_list[j]],cnt3,cnt1, cnt2, fscore,spearman, final);
			}
	...
	for (i = 0; i < rows; i++)
	{
		for (j = i+1; j < rows; j++)
		{
			...
			if (po->IS_spearman && cnt>5)
			{
				...
				fscore = get_f_socre (cnt, cnt1, cnt2);
				cnt = ceil (2*cnt*spearman*fscore);
                        	/*printf ("spearman\t%s\t%s\t%d\t%.2f\t%d\t%d\n",genes_n[i],genes_n[j],cnt,spearman,cnt1, cnt2);*/
			}
