...
-g : to-be-searched gene name, just consider the seeds containing current gene\n\
...
-n : the flag to use new descretization method regarding mixture normal distribution model\n\
-e : the number of iterations in EM algorithm of mixtured normal distribution when discretization\n\
...
void get_options (int argc, char* argv[])
{
	 ...
	 */
	while ((op = getopt(argc, argv, "i:b:q:r:dsf:k:o:c:g:G:SCl:m:e:pnRFh")) >0)
	{
		switch (op)
		{
			 ...
			 */
			case 'G': strcpy(po->FT, optarg); po->IS_Fname = TRUE; break;
